package body Matrices is
    function "*"
       (Left, Right : Matrix)
        return Matrix
    is
        Result : Matrix (Left'Range (1), Right'Range (2)) := (others => (others => Zero));
    begin
    ...
    function Invert
       (Source : Matrix)
        return Matrix
    is
        Expanded : Matrix (Source'Range (1), Source'First (2) .. Source'Last (2) * 2);
    ...
    is
        ...
        begin
            for Col in From'Range (2) loop
                From (Minuend, Col) := From (Minuend, Col) - From (Subtrahend, Col) * Factor;
            end loop;
        end Subtract_Rows;
    ...
    begin
        ...
        begin
            Result := Invert (Regressors_T * Regressors) * Regressors_T * To_Matrix (Source);
        end;
